...
const filteredReservasi = computed(() => {
  ...
  // 1. Search Logic (Nama, Username, Lomba)
...
const openModal = (data: ReservasiItem) => {
  if (data.status === 'BOOKED') return // Guard: Gabisa liat bukti kalau statusnya masih BOOKED
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      <div class="bg-white px-12 py-5 shadow-sm flex justify-between items-center shrink-0 font-plus-jakarta">
        ...
        <div class="text-right">
          <p class="font-semibold text-sm leading-tight text-gray-800">{{ authStore.user?.fullName }}</p>
      ...
      <div class="flex-1 overflow-y-auto px-12 py-8">
        ...
        <div class="flex flex-col gap-4 mb-6 font-plus-jakarta bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          <div class="grid grid-cols-3 gap-4">
            <select v-model="filterLomba" class="bg-gray-50 border border-gray-300 text-gray-700 text-sm rounded-lg p-2.5">
              <option value="">Semua Lomba</option>
              <option v-for="l in lombaOptions" :key="l.id" :value="l.namaLomba">{{ l.namaLomba }}</option>
            </select>

            <select v-model="filterStatus" class="bg-gray-50 border border-gray-300 text-gray-700 text-sm rounded-lg p-2.5">
            ...
            <select v-model="sortBy" class="bg-gray-50 border border-gray-300 text-gray-700 text-sm rounded-lg p-2.5">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden font-plus-jakarta">
          <table class="w-full text-left">
            ...
            <tbody>
              <tr v-if="loading"><td colspan="6" class="py-12 text-center">Memuat data...</td></tr>
              <tr v-else v-for="item in filteredReservasi" :key="item.id" class="border-b hover:bg-gray-50 transition-colors">
                <td class="py-4 px-6 text-xs text-center whitespace-pre-line">{{ new Date(item.waktuReservasi).toLocaleString('id-ID') }}</td>
                <td class="py-4 px-6 text-sm text-center">
                  <div class="font-bold">{{ item.namaPeserta }}</div>
                  <div class="text-xs text-gray-500">@{{ item.username }}</div>
                </td>
                <td class="py-4 px-6 text-sm text-center font-medium">{{ item.namaLomba }}</td>
                <td class="py-4 px-6 text-sm text-center text-gray-700">Rp {{ item.nominal.toLocaleString('id-ID') }}</td>
                ...
                <td class="py-4 px-6 text-sm text-center">
                  <div v-if="item.status === 'MENUNGGU_KONFIRMASI'" class="flex justify-center gap-2">
                    <button @click="openConfirmModal(item)" class="bg-[#4ade80] text-white px-3 py-1 rounded-md text-xs font-bold">Terima</button>
                    <button @click="prosesTolakDummy" class="bg-[#f87171] text-white px-3 py-1 rounded-md text-xs font-bold">Tolak</button>
                  </div>
                  <div v-else>
