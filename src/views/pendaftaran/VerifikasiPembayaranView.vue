...
const filteredReservasi = computed(() => {
  ...
  result.sort((a, b) => {
    switch (sortBy.value) {
      case 'date-desc':
        return new Date(b.waktuReservasi).getTime() - new Date(a.waktuReservasi).getTime()
      case 'date-asc':
        return new Date(a.waktuReservasi).getTime() - new Date(b.waktuReservasi).getTime()
      case 'name-asc':
        return a.namaPeserta.localeCompare(b.namaPeserta)
      case 'name-desc':
        return b.namaPeserta.localeCompare(a.namaPeserta)
      default:
        return 0
    }
  })
...
<template>
      ...
      <div class="bg-white px-12 py-5 shadow-sm flex justify-between items-center shrink-0">
        ...
        <div class="text-right">
          <p class="font-semibold text-sm leading-tight text-gray-800">
            {{ authStore.user?.fullName || 'Koordinator' }}
          </p>
      ...
      <div class="flex-1 overflow-y-auto px-12 py-8">
        <transition name="fade">
          <div
            ...
            "
            class="p-4 rounded-lg mb-4 text-sm font-medium flex items-center justify-between shadow-sm"
          >
            <span>{{ notification }}</span>
            <button @click="notification = ''" class="text-current opacity-70 hover:opacity-100">
              &times;
            </button>
          </div>
        </transition>
        ...
        <div
          class="flex flex-col gap-4 mb-6 bg-white p-6 rounded-xl shadow-sm border border-gray-100"
        >
          ...
          <div class="grid grid-cols-3 gap-4">
            <select
              v-model="filterLomba"
              class="bg-gray-50 border border-gray-300 text-gray-700 text-sm rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-blue-400"
            >
              <option value="">Semua Lomba</option>
              <option v-for="l in lombaOptions" :key="l.id" :value="l.namaLomba">
                {{ l.namaLomba }}
              </option>
            </select>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <table class="w-full text-left border-collapse">
            ...
            <tbody>
              ...
              <tr v-else-if="filteredReservasi.length === 0">
                <td colspan="6" class="py-12 text-center text-gray-500 font-medium">
              ...
              <tr
                v-else
                v-for="item in filteredReservasi"
                :key="item.id"
                class="border-b hover:bg-gray-50 transition-colors"
              >
                <td class="py-4 px-6 text-xs text-[#2E42B2] text-center whitespace-pre-line">
                  {{ new Date(item.waktuReservasi).toLocaleString('id-ID') }}
                </td>
                <td class="py-4 px-6 text-sm text-center">
                  <div class="font-bold text-[#2E42B2]">{{ item.namaPeserta }}</div>
                  <div class="text-xs text-gray-500">@{{ item.username }}</div>
                </td>
                <td class="py-4 px-6 text-sm text-[#2E42B2] text-center font-medium">
                  {{ item.namaLomba }}
                </td>
                <td class="py-4 px-6 text-sm text-center text-gray-700 font-semibold">
                ...
                <td class="py-4 px-6 text-sm text-center">
                  <div
                    v-if="item.status === 'MENUNGGU_KONFIRMASI'"
                    class="flex justify-center gap-2"
                  >
                    <button
                      @click="openConfirmModal(item)"
                      class="bg-[#4ade80] hover:bg-[#22c55e] text-white px-3 py-1.5 rounded-md text-xs font-bold transition-colors"
                    >
                      Terima
                    </button>
                    <button
                      @click="openTolakModal(item)"
                      class="bg-[#ef4444] hover:bg-[#dc2626] text-white px-3 py-1.5 rounded-md text-xs font-bold transition-colors"
                    >
                      Tolak
                    </button>
                  </div>
                  <div v-else>
                    <span
                      :class="[
                        'py-1 px-3 rounded-full text-[10px] font-bold border uppercase',
  ...
  <Teleport to="body">
    <div
      v-if="isModalOpen"
      class="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] font-plus-jakarta"
      @click.self="closeModal"
    >
      <div
        class="bg-white rounded-2xl w-[450px] overflow-hidden shadow-2xl flex flex-col max-h-[90vh]"
      >
        <div class="bg-[#eef2ff] p-5 text-center border-b border-gray-200">
          ...
          <div
            class="bg-[#d0d7f5] inline-block px-4 py-2 rounded-lg text-sm text-[#2E42B2] font-semibold text-left"
          >
            <p>Nama &nbsp;&nbsp;&nbsp;: {{ selectedData?.namaPeserta }}</p>
            <p>Nominal : {{ formatRupiah(selectedData?.nominal || 0) }}</p>
            <p v-if="selectedData?.keteranganTolak" class="text-red-700 mt-2 font-bold">
              Alasan Penolakan: {{ selectedData.keteranganTolak }}
            </p>
          </div>
        </div>
        ...
        <div class="p-6 overflow-y-auto flex-1 flex justify-center bg-gray-50">
          <img
            :src="selectedData?.urlBukti"
            alt="Bukti Transfer"
            class="max-w-full rounded-lg shadow-sm border border-gray-200 object-contain max-h-[400px]"
          />
        </div>
        ...
        <div class="p-5 border-t border-gray-200 flex justify-between items-center bg-white gap-4">
          <button
            @click="closeModal"
            class="px-6 py-2.5 rounded-lg border border-gray-300 text-gray-600 font-semibold hover:bg-gray-100 transition-colors w-full"
          >
            Tutup
          </button>
          <button
            v-if="selectedData?.status === 'MENUNGGU_KONFIRMASI'"
  ...
  <Teleport to="body">
    <div
      ...
      class="fixed inset-0 bg-black/50 flex items-center justify-center z-[110] font-plus-jakarta"
      @click.self="closeConfirmModal"
    >
      <div class="bg-white rounded-xl shadow-xl w-[450px] p-8 text-center mx-4">
        <h2 class="text-2xl font-bold text-[#2E42B2] mb-3">Konfirmasi Pembayaran</h2>
        <p class="text-gray-600 mb-8 font-medium">
          Pastikan bukti pembayaran dari <b>{{ itemToConfirm?.namaPeserta }}</b> sudah valid dan
  ...
  <Teleport to="body">
    <div
      v-if="isTolakModalOpen"
      class="fixed inset-0 bg-black/50 flex items-center justify-center z-[110] font-plus-jakarta"
      @click.self="closeTolakModal"
    >
      <div class="bg-white rounded-xl shadow-xl w-[450px] p-8 text-center mx-4">
        <h2 class="text-2xl font-bold text-[#2E42B2] mb-6">Beri Alasan Penolakan</h2>
