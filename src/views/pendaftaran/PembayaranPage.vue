<template>
  <div class="h-screen flex flex-col bg-white font-plus-jakarta overflow-hidden">
    ...
    <header_mobile title="Upload Pembayaran" @menu-click="isSidebarOpen = true" class="flex-none" />
    ...
    <main class="flex-1 overflow-y-auto px-4 md:px-8 py-5 flex flex-col bg-[#F8FAFC]">
      <div class="w-full max-w-md md:max-w-4xl mx-auto">
        <div class="w-full mb-4">
          <button
            @click="goBack"
            class="text-[#2E42B2] font-semibold text-sm flex items-center gap-2 hover:opacity-70 cursor-pointer transition"
          >
        ...
        <div
          ...
          class="flex flex-col items-center justify-center p-6 md:p-10 text-center mt-6 bg-white rounded-2xl border border-[#D9E3FF] shadow-sm max-w-xl mx-auto"
        >
          <div
            class="w-20 h-20 bg-green-500 rounded-full flex items-center justify-center mb-5 border-8 border-green-100 shadow-sm"
          >
            <svg
              class="w-10 h-10 text-white success-icon"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <h2 class="text-xl md:text-2xl font-black text-[#2E42B2] mb-2">
            Pembayaran berhasil<br />ter-upload!
          </h2>
          <p class="text-gray-500 text-xs md:text-sm mb-6">
            Tim kami sedang memverifikasi pembayaran Anda. Mohon tunggu maksimal 1x24 jam.
          </p>

          <button
            @click="goToMyTicket"
            class="btn-desktop w-full max-w-xs mx-auto rounded-xl bg-[#2E42B2] py-3 text-sm font-bold text-white shadow-md hover:bg-blue-800 transition cursor-pointer"
          >
            Lihat Tiket Saya
          </button>
        </div>
        ...
        <div v-else class="flex flex-col md:flex-row gap-5 pb-24 md:pb-6">
          <div class="w-full md:w-1/2 flex flex-col space-y-4">
            <div class="bg-[#2E42B2] text-white text-center py-5 px-4 rounded-xl shadow-sm">
              <p class="text-xs text-blue-100 mb-1">
                Waktu tersisa sebelum <span class="font-bold">{{ deadlineText }}</span>
              </p>
            ...
            <div class="bg-white p-3 rounded-xl flex items-center border border-gray-100 shadow-sm">
              <div
                class="w-10 h-10 bg-[#eef2ff] text-[#2E42B2] rounded-lg flex items-center justify-center mr-3 shrink-0"
              >
                <svg
                  class="w-6 h-6"
              ...
              <div
                class="bg-[#2E42B2] text-white text-center px-3 py-1.5 rounded-lg shadow-sm shrink-0"
              >
              ...
              <div class="space-y-2.5 text-xs">
                ...
                <div class="flex justify-between items-center">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <button
                      @click="copyText('5405213556')"
                      class="bg-gray-100 text-[#2E42B2] px-2 py-0.5 rounded text-[10px] font-bold hover:bg-gray-200 transition cursor-pointer"
                    >
                      Salin
                    </button>
                  </div>
                ...
                <div class="mt-3 p-3 border border-orange-200 bg-orange-50 rounded-lg">
                  ...
                  <div class="flex justify-between items-center">
                    ...
                    <button
                      @click="copyText(nominal.toString())"
                      class="bg-white border border-gray-200 text-gray-600 px-2 py-1 rounded text-[10px] font-bold hover:bg-gray-50 shadow-sm transition cursor-pointer"
                    >
                      Salin
                    </button>
                  </div>
            ...
            <div
              class="bg-blue-50 border border-blue-100 text-[#2E42B2] text-[11px] p-2.5 rounded-lg flex items-start"
            >
              <svg
              ...
              <p class="font-medium leading-relaxed">
                Transfer tepat sesuai nominal. Nominal berbeda dapat memperlambat proses verifikasi.
              </p>
            </div>
          </div>
          ...
          <div class="w-full md:w-1/2 flex flex-col space-y-4">
            <div
              class="bg-white border border-gray-100 rounded-xl p-5 shadow-sm flex flex-col h-full"
            >
              <h3 class="text-sm font-bold text-[#2E42B2] mb-1">Unggah Bukti</h3>
              <p class="text-[11px] text-gray-500 mb-4">
                Pastikan gambar jelas. (JPG, PNG, PDF Maks 2MB)
              </p>

              <label
                class="flex-1 flex flex-col items-center justify-center min-h-[160px] border-2 border-dashed border-[#2E42B2] bg-[#f8faff] rounded-lg hover:bg-[#eef2ff] transition cursor-pointer relative p-4 text-center"
              >
                <svg
                  class="w-8 h-8 mb-2 text-[#2E42B2]"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <span
                  v-if="selectedFile"
                  class="mt-2 bg-green-100 text-green-700 px-3 py-1 rounded-full text-[10px] font-bold truncate max-w-[200px] block"
                >
                  ✓ {{ selectedFile.name }}
                </span>
              ...
              <p
                v-if="errorMessage"
                class="text-[11px] font-bold text-red-500 bg-red-50 p-2 rounded-lg mt-3 text-center"
              >
                {{ errorMessage }}
              </p>
            </div>
            ...
            <button
              ...
              class="btn-desktop w-full py-3.5 rounded-xl text-white font-bold text-sm shadow-md transition-all active:scale-95 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed enabled:bg-[#2E42B2] enabled:hover:bg-blue-800 cursor-pointer"
            >
    ...
    <div
      class="btn-mobile-wrapper flex-none p-4 bg-white border-t border-gray-100 shadow-[0_-4px_15px_rgba(0,0,0,0.05)] z-20"
    >
      <div class="w-full max-w-md mx-auto">
        <button
          v-if="showSuccessView"
          @click="goToMyTicket"
          class="w-full py-3.5 rounded-xl bg-[#2E42B2] hover:bg-blue-800 text-white font-bold text-sm shadow-md transition-all active:scale-95 cursor-pointer"
        >
          Lihat Tiket Saya
        </button>
        <button
          ...
          @click="submitBukti"
          :disabled="!selectedFile || isSubmitting"
          class="w-full py-3.5 rounded-xl text-white font-bold text-sm shadow-md transition-all active:scale-95 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed enabled:bg-[#2E42B2] enabled:hover:bg-blue-800 cursor-pointer"
        >
          {{ isSubmitting ? 'Memproses...' : 'Kirim Bukti Pembayaran' }}
        </button>
      </div>
...
const reservasiId = ref(readQueryString(route.query.reservasiId) || String(route.params.id ?? ''))
...
onMounted(() => {
  if (!reservasiId.value) {
    errorMessage.value =
      'ID reservasi tidak ditemukan. Silakan buka dari Tiket Saya atau Reservasi Gantangan.'
  }
