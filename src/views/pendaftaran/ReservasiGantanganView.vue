<template>
  ...
  <div class="h-screen flex flex-col bg-white font-plus-jakarta overflow-hidden">
    ...
    <main class="flex-1 overflow-y-auto px-6 py-4 flex flex-col items-center">
      <div class="w-full max-w-md">
        ...
        <div class="w-full mb-4">
          <button
            @click="router.back()"
            class="text-[#2E42B2] font-semibold text-[16px] flex items-center gap-2 hover:opacity-70 cursor-pointer"
          >
        ...
        <h2
          class="text-[#2E42B2] text-[17px] font-bold text-center mb-6 uppercase leading-tight px-4"
        >
        ...
        <div class="relative p-2 flex justify-center">
          <!-- Garis Salib Tengah -->
          <div
            class="absolute left-1/2 top-0 bottom-0 w-[2px] bg-[#2E42B2]/20 -translate-x-1/2 z-0"
          ></div>
          <div
            class="absolute left-0 right-0 top-1/2 h-[2px] bg-[#2E42B2]/20 -translate-y-1/2 z-0"
          ></div>

          <div class="grid grid-cols-2 gap-x-12 gap-y-10 relative z-10">
            <!-- Blok II -->
            <div class="grid grid-cols-2 gap-3">
              <p class="col-span-2 text-center text-[12px] font-bold text-[#2E42B2] uppercase mb-1">
                Blok II
              </p>
              <SeatButton
                v-for="n in [24, 23, 17, 18, 16, 15]"
                :key="n"
                :nomor="n"
                :status="getSeatStatus(n)"
                :isSelected="selectedSeat === n"
            ...
            <div class="grid grid-cols-2 gap-3">
              <SeatButton
                ...
                @click="handleSelect(n)"
              />
              <p class="col-span-2 text-center text-[12px] font-bold text-[#2E42B2] uppercase mt-1">
                Blok I
              </p>
            </div>
            <!-- Blok IV -->
    ...
    <div
      class="flex-none p-6 bg-white border-t border-gray-100 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]"
    >
      <div class="max-w-md mx-auto">
        <button
          @click="handleLanjutPembayaran"
          :disabled="!selectedSeat || loading"
          class="w-full py-4 rounded-2xl text-white font-bold text-[16px] shadow-lg transition-all active:scale-95 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed enabled:bg-[#2E42B2] enabled:hover:bg-blue-800 cursor-pointer"
        >
...
const handleLanjutPembayaran = async () => {
  ...
  try {
    const cleanLombaId = lombaId.startsWith(':') ? lombaId.substring(1) : lombaId
    const response = await postBookSeat({
      lombaId: cleanLombaId,
      nomorGantangan: selectedSeat.value,
    })
