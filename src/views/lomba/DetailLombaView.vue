...
// Warna Status: Berlangsung (Hijau #5CB85C), Belum Dimulai (Kuning #FFB800)
...
const buttonStatus = computed(() => {
  if (userRole.value === 'JURI') {
    return { text: lomba.value.canStartJudging ? 'MULAI PENILAIAN' : 'PENILAIAN BELUM DIBUKA', disabled: !lomba.value.canStartJudging, theme: 'blue' };
  }
...
const handleButtonClick = () => {
  ...
  if (resStatus === 'BOOKED' || resStatus === 'MENUNGGU_KONFIRMASI') {
    ...
    router.push(`/judging/select-gantangan/${lombaId}`);
  } else {
...
const goToEdit = () => {
  router.push(`/edit-lomba/${lombaId}`);
};
const confirmDelete = async () => {
  if (confirm('Yakin ingin menghapus lomba ini?')) {
    try { await deleteLomba(lombaId); router.push('/katalog-lomba'); } catch (e) { alert('Gagal menghapus'); }
  }
};
const handleToggleStatus = async () => {
  if (lomba.value.status !== 'BELUM_DIMULAI') return;
  if (confirm('Ubah status lomba menjadi BERLANGSUNG?')) {
    try { await updateLombaStatus(lombaId, 'BERLANGSUNG'); await loadData(); } catch (e) { alert('Gagal update status'); }
  }
};
