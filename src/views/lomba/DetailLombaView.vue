  ...
  <div class="min-h-screen font-plus-jakarta flex flex-col bg-[#F4F7FE]">
      ...
      <div class="flex-1 flex flex-col min-w-0">
        ...
        <main class="flex-1 overflow-y-auto p-10">
          ...
          <transition name="fade">
            <div v-if="notification" :class="notificationType === 'success' ? 'bg-green-100 text-green-800 border-green-300' : 'bg-red-100 text-red-800 border-red-300'" class="p-4 rounded-lg mb-8 text-sm font-bold flex items-center justify-between shadow-sm border">
              <span>{{ notification }}</span>
              <button @click="notification = ''" class="text-current opacity-70 hover:opacity-100 cursor-pointer">&times;</button>
            </div>
          </transition>
          ...
          <div v-else class="border border-[#2E42B2] rounded-[24px] bg-white p-10 shadow-sm relative">
              ...
              <div class="flex flex-col gap-3">
                ...
                <template v-else-if="userRole === 'PESERTA' || userRole === 'JURI'">
                  ...
                  <div class="mt-2 text-center w-48">
                    <p v-if="buttonStatus.text === 'KUOTA PENUH'" class="text-[11px] text-[#6D9BED] font-bold">Maaf, seluruh gantangan sudah terisi.</p>
                    <p v-else-if="buttonStatus.text === 'RESERVASI DITUTUP'" class="text-[11px] text-[#6D9BED] font-bold">Batas waktu reservasi telah berakhir.</p>
                  </div>
                </template>
              </div>
            </div>
            ...
            <div class="flex flex-wrap items-end gap-6 mb-10">
              <div class="flex flex-col gap-1.5">
                ...
                <div class="border border-[#2E42B2] rounded-xl px-4 py-3 flex items-center gap-3 bg-[#DEE8FB] min-w-[200px]">
            ...
            <div v-if="userRole === 'JURI'" class="bg-[#DEE8FB] border border-[#2E42B2]/20 rounded-[16px] p-8 grid grid-cols-3 gap-10">
              <div><p class="text-sm font-bold text-[#2E42B2] mb-1">Tersisa:</p><p class="text-[15px] font-semibold text-[#2D48C8]">{{ availableCount }} / {{ lomba.jumlahGantangan || 24 }}</p></div>
              <div><p class="text-sm font-bold text-[#2E42B2] mb-1">Jenis Burung:</p><p class="text-[15px] font-semibold text-[#2D48C8] uppercase">{{ lomba.jenisBurung?.replace(/_/g, ' ') }}</p></div>
              <div><p class="text-sm font-bold text-[#2E42B2] mb-1">Koordinator Lomba:</p><p class="text-[15px] font-semibold text-[#2D48C8]">{{ lomba.contactPerson }}</p></div>
            </div>

            <!-- Desktop Peserta, Admin, Koor Pend (Sembunyikan Juri) -->
            <div v-else class="bg-[#DEE8FB] border border-[#2E42B2]/20 rounded-[16px] p-8 grid grid-cols-4 gap-10">
              <div>
              ...
              <div class="space-y-6">
                ...
                <div><p class="text-sm font-extrabold text-[#2E42B2] mb-1">Narahubung:</p><p class="text-[15px] font-semibold text-[#2D48C8]">{{ lomba.contactPerson }}</p></div>
              </div>
              <div class="space-y-6">
                <div><p class="text-sm font-extrabold text-[#2E42B2] mb-1">Tersisa:</p><p class="text-[15px] font-semibold text-[#2D48C8]">{{ availableCount }} / {{ lomba.jumlahGantangan || 24 }}</p></div>
                <div><p class="text-sm font-extrabold text-[#2E42B2] mb-1">Harga Tiket:</p><p class="text-[15px] font-semibold text-[#2D48C8]">{{ formatHarga(lomba.hargaTiket) }}</p></div>
              </div>
              <div class="space-y-6">
                <div><p class="text-sm font-bold text-[#2E42B2] mb-1">Jenis Burung:</p><p class="text-[15px] font-semibold text-[#2D48C8]">{{ lomba.jenisBurung?.replace(/_/g, ' ') }}</p></div>
              </div>
            </div>
          </div>
        </main>
    ...
    <div v-else class="flex flex-col min-h-screen bg-white pb-32">
      ...
      <main class="p-6 overflow-y-auto">
        <transition name="fade">
          <div v-if="notification" :class="notificationType === 'success' ? 'bg-green-100 text-green-800 border-green-300' : 'bg-red-100 text-red-800 border-red-300'" class="p-4 rounded-lg mb-6 text-sm font-bold flex items-center justify-between shadow-sm border">
            <span>{{ notification }}</span>
            <button @click="notification = ''" class="text-current opacity-70 hover:opacity-100 cursor-pointer">&times;</button>
          </div>
        </transition>
          ...
          <div v-if="userRole === 'JURI'" class="border-2 border-[#DEE8FB] rounded-[32px] p-6 shadow-sm space-y-6">
            ...
            <div class="space-y-4">
              ...
              <div class="space-y-1">
                ...
                <div class="bg-[#DEE8FB] border border-[#2E42B2] p-3 rounded-xl flex items-center gap-3 text-[13px] font-bold text-[#1C244F]">
                  <Icon icon="ph:calendar-blank-fill" class="w-5 h-5 text-[#2E42B2]" />
            ...
            <div class="space-y-4">
              ...
              <div class="space-y-1"><span class="text-[12px] font-bold text-[#2E42B2] ml-1">Koordinator Lomba:</span>
                <div class="bg-[#DEE8FB] border border-[#2E42B2] p-4 rounded-xl flex items-center gap-3 text-[14px] font-bold text-[#1C244F]">
          ...
          <div v-else class="space-y-6">
            <div class="border-2 border-[#DEE8FB] rounded-[24px] p-6 shadow-sm flex flex-col">
              ...
              <div class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div class="space-y-1"><span class="text-[12px] font-bold text-[#2E42B2] ml-1">Kelas:</span>
                    <div class="bg-[#DEE8FB] border border-[#2E42B2] p-3 rounded-xl flex items-center gap-2 text-[13px] font-bold text-[#1C244F]">
                      <Icon icon="ph:identification-badge-fill" class="w-5 h-5 text-[#2E42B2]" /><span class="truncate">{{ lomba.kelas }}</span>
                    </div>
                  </div>
                  <div class="space-y-1"><span class="text-[12px] font-bold text-[#2E42B2] ml-1">Jenis Burung:</span>
                    <div class="bg-[#DEE8FB] border border-[#2E42B2] p-3 rounded-xl flex items-center gap-2 text-[13px] font-bold text-[#1C244F]">
                      <Icon icon="ph:bird-fill" class="w-5 h-5 text-[#2E42B2]" /><span class="truncate uppercase">{{ lomba.jenisBurung?.replace(/_/g, ' ') }}</span>
                    </div>
                  </div>
                </div>
            ...
            <div class="border-2 border-[#DEE8FB] rounded-[24px] p-6 space-y-6 shadow-sm">
              <div><p class="text-[12px] font-bold text-[#2E42B2] mb-3 tracking-wider">Daftar Hadiah:</p>
              ...
              <div><p class="text-[12px] font-bold text-[#2E42B2] mb-3 tracking-wider">Narahubung:</p>
      ...
      <div v-if="userRole === 'PESERTA' || userRole === 'JURI'" class="fixed bottom-0 left-0 right-0 p-6 bg-white border-t border-gray-100 shadow-[0_-8px_20px_rgba(0,0,0,0.08)] z-30">
        <div class="max-w-md mx-auto text-center">
          <button :disabled="buttonStatus.disabled" @click="handleButtonClick" class="w-full py-4 rounded-xl font-bold text-lg shadow-lg"
            :class="[
              buttonStatus.theme === 'success' ? 'bg-[#AFF4C6] text-[#02542D] border border-[#02542D]' : '',
              buttonStatus.theme === 'warning' ? 'bg-[#FFD88A] text-[#BF6A02] border border-[#BF6A02] active:scale-95' : '',
              buttonStatus.theme === 'blue' && !buttonStatus.disabled ? 'bg-[#2E42B2] text-white active:scale-95 hover:bg-blue-800' : '',
          ...
          <div class="mt-3">
            <p v-if="buttonStatus.text === 'KUOTA PENUH'" class="text-[13px] text-[#6D9BED] font-medium">Maaf, seluruh gantangan sudah terisi.</p>
            <p v-else-if="buttonStatus.text === 'RESERVASI DITUTUP'" class="text-[13px] text-[#6D9BED] font-medium">Batas waktu reservasi telah berakhir.</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <Teleport to="body">
      <div v-if="isDeleteModalOpen" class="fixed inset-0 bg-black/40 flex items-center justify-center z-[100]" @click.self="closeDeleteModal">
        <div class="bg-white rounded-2xl p-8 w-[360px] mx-4 shadow-lg border border-gray-200 font-plus-jakarta text-center">
          ...
          <div class="flex gap-3">
            <button @click="closeDeleteModal" class="flex-1 py-2.5 rounded-xl border border-gray-300 text-gray-600 text-sm font-semibold hover:bg-gray-50 transition-colors cursor-pointer">
              Kembali
            </button>
            <button @click="confirmDelete" class="flex-1 py-2.5 rounded-xl bg-[#2E42B2] text-white text-sm font-semibold hover:bg-[#1c2d8f] transition-colors cursor-pointer">
              Hapus
            </button>
          </div>
        </div>
...
const getStatusStyle = (s: string) => {
  switch (s) {
    case 'BERLANGSUNG': return { bg: '#5CB85C' };   // HIJAU
    case 'BELUM_DIMULAI': return { bg: '#FFB800' }; // KUNING
    case 'SELESAI': return { bg: '#828282' };       // ABU
    case 'DIBATALKAN': return { bg: '#D9534F' };    // MERAH
    default: return { bg: '#DEE8FB' };
  }
};
...
const buttonStatus = computed(() => {
  ...
  if (lomba.value.status !== 'BELUM_DIMULAI' || !lomba.value.isReservable) return { text: 'RESERVASI DITUTUP', disabled: true, theme: 'disabled' };
