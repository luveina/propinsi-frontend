  ...
  <div class="flex flex-col h-screen bg-[#F2F2F2] font-plus-jakarta overflow-hidden">
    ...
    <main class="flex-1 p-5 overflow-y-auto flex flex-col items-center">
      ...
      <div v-if="loading" class="flex-1 flex items-center justify-center">
        <div
          class="animate-spin rounded-full h-10 w-10 border-4 border-[#3041b3] border-t-transparent"
        ></div>
      </div>
      ...
      <div
        v-else
        class="w-full max-w-md bg-white rounded-2xl shadow-sm border border-blue-200 overflow-hidden mb-6"
      >
        <table class="w-full text-center border-collapse">
          ...
          <tbody class="text-blue-900">
            <tr
              ...
              :key="index"
              class="border-b border-blue-50 odd:bg-white even:bg-blue-50/30"
            >
              <td class="py-3 text-xs font-semibold">{{ index + 1 }}</td>
              <td class="py-3 flex justify-center">
                <span
                  class="bg-[#9CBFF4] text-[#3041b3] px-5 py-0.5 rounded-full border border-[#3041b3] font-black text-sm"
                >
                  {{ item.nomorGantangan }}
                </span>
              </td>
