...
const fetchAccounts = async () => {
  try {
    ...
    accounts.value = data; // Masukkan data dari backend ke variabel tabel
  } catch (error) {
...
const filteredAccounts = computed(() => {
  return accounts.value.filter((account) => {
    // Cek apakah username cocok dengan ketikan di kolom pencarian
    const matchSearch = account.username?.toLowerCase().includes(searchQuery.value.toLowerCase()) || false;
    // Cek apakah rolenya cocok dengan pilihan dropdown
...
const handleSuccessAdd = () => {
  ...
  fetchAccounts(); // Refresh isi tabel biar akun baru langsung muncul!
};

const handleSuccessUpdate = () => {
  isModalOpen.value = false; // Tutup modal
  fetchAccounts(); // Refresh isi tabel biar akun yang diedit langsung terupdate!
};

// Helper terpusat yang dipanggil oleh modal kapan pun ia "success"
...
const openEditModal = (account: any) => {
  selectedAccount.value = { ...account }; // Clone data agar tidak ngerusak tabel sebelum disave
...
<template>
  <div class="w-full min-h-screen bg-gray-100">
    <!-- Header Section -->
    <div class="bg-white px-12 py-6 shadow-sm flex justify-between items-center shadow-b">
    ...
    <div class="px-12 py-8">
      <div v-if="notification" :class="notificationType === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'" class="p-3 rounded mb-4">
        {{ notification }}
      </div>
          ...
          >
            ...
            <td class="py-3 px-6 text-sm text-center">
              <div class="flex justify-center gap-2">
                <button v-if="account.status !== 'Inactive'" @click="openEditModal(account)"
                class="bg-[#FCD34D] hover:bg-yellow-600 text-yellow-900 px-3 py-1 rounded text-xs font-medium transition-colors cursor-pointer">Edit</button>
                <button v-if="account.status !== 'Inactive'" @click="openDeleteModal(account)" class="bg-[#EF4444] hover:bg-red-700 text-white px-3 py-1 rounded text-xs font-medium transition-colors cursor-pointer">Hapus</button>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <div v-if="isDeleteModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/80">
      <div class="bg-white rounded-lg shadow-lg w-full max-w-sm p-6">
        ...
        <div class="mt-6 flex justify-end">
          <button @click="confirmDeleteAccount" class="px-4 py-2 bg-blue-700 text-white rounded hover:bg-blue-800 cursor-pointer">Ya, Nonaktifkan Akun</button>
        </div>
      </div>
    </div>
