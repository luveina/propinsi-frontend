...
import type {
  ...
  BookingRequest,
  BookingResponse,
} from '@/interfaces/reservasi.interface'
...
export const getDenahGantangan = async (
  lombaId: string,
): Promise<BaseResponse<GantanganItem[]>> => {
...
export const postBookSeat = async (
...
export const postUploadBukti = async (
  reservasiId: string,
  file: File,
...
export const verifyReservasi = async (
  id: string,
  status: 'PAID' | 'Invalid' | 'REJECTED',
  komentar?: string,
) => {
  try {
